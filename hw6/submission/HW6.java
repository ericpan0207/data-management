...
public class HW6 {
  ...
  public static void main(String[] args) {
    ...
    // comment out the following two lines once you started working on the problems. They are for demonstration only.
    ...
    r1.javaRDD().repartition(1).saveAsTextFile(output + "1");

    /* Problem 2 */
    JavaRDD<Row> r2 = Q2(spark, dataFile);
    r2.repartition(1).saveAsTextFile(output + "2");

    /* Problem 3 */
    JavaPairRDD<Tuple2<String, Integer>, Integer> r3 = Q3(spark, dataFile);
    r3.repartition(1).saveAsTextFile(output + "3");
    ...
    spark.createDataset(Collections.singletonList(r4), Encoders.tuple(Encoders.STRING(), Encoders.INT()))
        .javaRDD().saveAsTextFile(output + "4");

    /* Problem 5 */
    JavaPairRDD<String, Double> r5 = Q5(spark, dataFile);
    r5.repartition(1).saveAsTextFile(output + "5");
  ...
  public static JavaRDD<Row> Q2 (SparkSession spark, String dataFile) {
    ...
    return d.filter(t -> t.get(ORIGIN_CITY_NAME).equals("Seattle, WA"))
  ...
  public static JavaPairRDD<Tuple2<String, Integer>, Integer> Q3 (SparkSession spark, String dataFile) {
    ...
    return d.filter(t -> t.getInt(CANCELLED) == 0)
     .mapToPair(t -> {
         Tuple2<String, Integer> key = new Tuple2<>(t.getString(ORIGIN_CITY_NAME), t.getInt(MONTH));
         Tuple2<Tuple2<String, Integer>, Integer> output = new Tuple2<>(key, 1);
         return output;
     })
     .reduceByKey((count, n) -> count + n);

  }
  
  static class TupleComparator implements Serializable, Comparator<Tuple2<String, Integer>> {
    @Override
  ...
  public static Tuple2<String, Integer> Q4 (SparkSession spark, String dataFile) {
    ...
    return d.map(t -> RowFactory.create(t.get(ORIGIN_CITY_NAME), t.get(DEST_CITY_NAME)))
     .distinct()
     .mapToPair(t -> {
          Tuple2<String, Integer> output = new Tuple2<>(t.getString(0), 1);
          return output;
     })
     .reduceByKey((count, n) -> count + n)
  ...
  public static JavaPairRDD<String, Double> Q5 (SparkSession spark, String dataFile) {
    ...
    return d.filter(t -> !t.isNullAt(DEP_DELAY))
      .mapToPair(t -> new Tuple2<>(t.getString(ORIGIN_CITY_NAME), new Tuple2<>(t.getInt(DEP_DELAY), 1)))
      .reduceByKey((t1, t2) -> new Tuple2<>(t1._1() + t2._1(), t1._2() + t2._2()))
      .mapValues((val) -> val._1() * 1.0/ val._2());
  }
